...
export default function EventBlock({ event, column, totalColumns, selected, onSelect }: EventBlockProps) {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'absolute rounded-md px-1.5 py-0.5 text-[10px] overflow-hidden cursor-pointer transition-all',
      ...
      onClick={(e) => {
        e.stopPropagation();
        if (ref.current) {
          onSelect(event, ref.current.getBoundingClientRect());
        }
      }}
    >
